...
//    }
    ...
    for (int x = 0; x < colsm; x++) {
        ...
        if (img_fixed.inImage(new_y, new_x)) {
...
//                int temp = (img_moved.valueAt(y, x, 0) / (256 / binN)) * binN + (img_fixed.valueAt(new_y, new_x, 0) / (256 / binN));
            int binY = (int) binN * img_moved.valueAt(y, x, 0) / 256;
            int binX = (int) binN * img_fixed.valueAt(new_y, new_x, 0) / 256;
            atomicAdd(&pxy.at<float>(binY, binX), 1.0f / (colsm * rowsm));
            atomicAdd(&py.at<float>(binY, 0), 1.0f / (colsm * rowsm));
//
...
//            float tempPxy = test_pxy[binIdx * binN + x]; // numPix > 0, pxy[i] > 0
            float& tempPxy = pxy.at<pixType>(binIdx, x);
            float& tempPy = py.at<pixType>(binIdx, 0);
            float& tempPx = px.at<float>(x, 0);
