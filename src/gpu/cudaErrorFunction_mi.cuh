...
/*
Function to multiply to matricies
INPUTS:
    Nx1 float* A
    1xN float* B
    NxN float* C - Output
...
CUDAFUNCTION float mutualInformation(unsigned int* pxy, int cols, int rows, int binN) {
    ...
    */
    ...
    for(int i = 0; i < binN*binN; i++) {
        float tempP = pxy[i]/numPix;
        if (pxy[i] > 0) {
            float tempTemp = tempP * log2(tempP / px_py[i]);
            if(tempTemp != INFINITY) temp += tempTemp;
            //DEBUG
...
*/
CUDAFUNCTION void histogram2d(unsigned int* hist2d, unsigned char * image1, unsigned char * image2, int cols, int rows, int binN) {
    ...
    for(int x = 0; x < cols; x++) {
        for(int y = 0; y < rows; y++) {
            ...
            if(temp >= 0 && temp < binN*binN) hist2d[temp] += 1;
        }
    }
}
...
grid_mi(nv_ext::Vec<grid_precision, D> &H, 
    ...
    for (int x = 0; x < colsm; x++) {
        for (int y = 0; y < rowsm; y++) {
            int new_x = (int)(tempH[3]*(x-colsm/2)*cos(H[2]) - (y-rowsm/2)*sin(tempH[2]) + tempH[0] + tempH[3]*colsm/2);
            int new_y = (int)((x-colsm/2)*sin(tempH[2]) + tempH[3]*(y-rowsm/2)*cos(tempH[2]) + tempH[1] + tempH[3]*rowsm/2);
            if ((new_x >= 0 && new_x < colsf) && (new_y >= 0 && new_y < rowsf))tempImage[new_y * colsf + new_x] = img_moved[y * colsm + x];
        }
    }
